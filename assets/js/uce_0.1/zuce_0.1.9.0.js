	...
	****************************************************/
	    ...
	    var Zuce = function(k,p)
	    {
	        ...
	        */
	        this.implement = function(eventType, fn){
	        	try{
		            ...
		            {
			        	//First, resets the before and after triggers of the use case
		            ...
		            if(!this.exists(name))
		            {
	                    ...
	                    if(bindToEventType)
	                        this.bind(bindToEventType, { eventtoraise: 'uc_' + this.name + '_' + name, preventPropagation: preventPropagation, preventDefault: preventDefault }, function(e){
	                        	if(e.data.preventPropagation == true)
	                        		e.stopPropagation();
	        ...
	        */
	        this.raise = function(eventType, usecase, originatingEvent){
	            try{
	                ...
	                if(usecase && usecase.name == 'ready')
                        ...
                        {
                        	this.log("Executing load function...");
	                        usecase.load();
                        }
	            } catch (e) {
	        ...
	        this.ready = function(fn, syncCall, timeOut){
	            try{
	            	if(typeof(fn) == "function")
	            	{
	            		...
	            		{
	            			this.fn = fn;
	            			//setTimeout("if($('body').data('events')) { var _vvv = $('body').data('events');for(var v in _vvv){if(_vvv[v][0].type.indexOf('" + this.name + "') > 0){alert(1);break;};}  uce('" + this.name + "').implement('ready').raise('ready');}", timeOut? timeOut : 0);
